...
Let us do an example using `% 10` (mod 10):
...
Showcasing, the value being return is ranging from `0 ~ 9`

For "ensuring" a positive range we use the following:

```c++
random_val = (std::rand() % 10 + 10) % 10;
```

There are various other methods for ranges using random number generation and modulus, you can look
up those methods independently.

> **However,** this method of random number generation is not recommended for Modern C++.
We were merely showcasing a legacy method that may appear in legacy code. This method
was dropped for various reasons. The `rand()` function has issues regarding the quality
of randomness, its limited functionality, and the availability of a newer library —`<random>`— meant
to address the previous issues. 

`rand()` uses global shared state, making it difficult to manage random sequences in different 
parts of a program. It also introduces potential problems in multithreaded environments, where
different threads might interfere with each other's random number generation. The numbers
generated by `rand()` are not very random and can be predictable if the seed is known. The
range `rand()` generates is limited to `[0, RAND_MAX]`, which may not be sufficient for some
applications.

Let us now look at the `random` library that addresses these problems.
